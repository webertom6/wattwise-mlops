    ...
    for row in rows.rows.values():
        ...
        for cf, columns in row.cells.items():
            for col, cells in columns.items():
                ...
                row_dict[clean_col.strip()] = cells[0].value.decode("utf-8", errors="ignore")
...
if selected_country:
    ...
    data_input_range_date = data_input[(data_input['Date'] >= range_date[0]) & (data_input['Date'] <= range_date[1])]
    ...
    option_stat = st.multiselect(
        ...
        ['Inputs', 'Predictions', 'Exponential Moving Average', 'Same month previous year', 'Confidence interval (predictions)'],
    ...
    if option_stat and 'Exponential Moving Average' in option_stat:
        data_input_range_date['Ema'] = data_input_range_date[option_feature].ewm(span = 30).mean()
    ...
    if option_stat and 'Same month previous year' in option_stat:
        ...
        previous_input_month = data_input[(data_input['Date'] >= min_date_prediction) & (data_input['Date'] < max_date_prediction)]
        previous_input_month['Shift Date'] = previous_input_month['Date'] + pd.DateOffset(years = max_date_input.year - option_year_comparison)
    ...
    if 'Confidence interval (predictions)'in option_stat:
        fig.add_trace(
            go.Scatter(
                ...
                y = pd.concat([data_predictions[option_feature+'_upper_bound'], data_predictions[option_feature+'_lower_bound'][::-1]]),
                fill = 'toself',
    ...
    fig.update_layout(
        title = option_feature,
        xaxis_title = 'Date',
        yaxis_title = option_feature
    )
    ...
    # Data with range filter
    data_input_range_date_meteo = data_input[(data_input['Date'] >= range_date_meteo[0]) & (data_input['Date'] <= range_date_meteo[1])]
    ...
    if option_stat_meteo and 'Exponential Moving Average' in option_stat_meteo:
        data_input_range_date_meteo['Ema'] = data_input_range_date_meteo[option_feature].ewm(span = 30).mean()
        data_predictions['Ema'] = data_predictions[option_feature].ewm(span = 30).mean()
    ...
    fig_corr = px.imshow(
        data_input.loc[:, data_input.columns != 'Date'].corr()
    )
