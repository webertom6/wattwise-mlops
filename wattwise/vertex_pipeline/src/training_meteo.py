...
@component(
    ...
    output_component_file="training_meteo.yaml",
)
def training_meteo(
    cfg: dict,
    preprocessed_dataset: Input[Dataset],
    preprocessed_target_meteo: Input[Dataset],
    model_meteo: Output[Artifact],
    metrics_meteo: Output[Metrics],
):
    """
    ...
    for blob in blobs:
        ...
        if filename.endswith(".pth"):
            # if filename containes word "meteo" whatver the case
            if "meteo" in filename.lower():
                ...
                logging.info(
                    f"Skipping file: {filename} as it does not contain 'meteo'"
                )
    ...
    optimizer = optim.Adam(gru_model_meteo.parameters(), lr=cfg["learning_rate"])
    logging.info(f"Loss function: {criterion}")
    logging.info(f"Optimizer: {optimizer}")

    model_name = gru_model_meteo.__class__.__name__
    ...
    for epoch in range(epochs):
        ...
        for idx, (X_batch, y_batch) in enumerate(train_loader):
            ...
            outputs, h0 = gru_model_meteo(X_batch, h0)
    ...
    with open(metrics_meteo.path, "w") as f:
    ...
    logging.info(f"Metrics saved to: {metrics_meteo.path}")

    # 6. Save the model
    # Save the PyTorch model weights to a .pth file
    torch.save(gru_model_meteo.state_dict(), model_meteo.path)
    logging.info(f"Model weights saved to: {model_meteo.path}")
